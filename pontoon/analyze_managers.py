...
with open(args.csv_path) as csv_file:
    ...
    for row in csv_reader:
        ...
        days_from_login = (
            ...
            datetime.datetime.strptime(row['Last Login'], '%Y-%m-%d')).days
        days_from_activity = (-1 if row['Last Activity'] == 'No activity yet' else (
            datetime.datetime.today() - datetime.datetime.strptime(row['Last Activity'], '%Y-%m-%d')).days)
        data[locale].append(
            {
...
print('Number of managers who have not logged in for more than {} days: {}'.format(inactivity_days, inactive))
