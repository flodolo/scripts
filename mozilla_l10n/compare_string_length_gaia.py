...
''' This script assumes that Gaia localizations are stored in a
...
def main():
    locales_folder = os.path.join(os.getcwd(), 'gaia-l10n')
    repo_english = os.path.join(locales_folder, 'en-US') + os.sep
    ...
    for locale in sorted(os.listdir(locales_folder)):
        if not locale.startswith('.'):
            ...
            repo_locale = os.path.join(locales_folder, locale) + os.sep
            directories = ['apps', 'shared']
            ...
            for directory in directories:
                if os.path.isdir(os.path.join(repo_locale, directory)):
                	...
                	l10n_package_english = rcs_client_english.get_package(repo_english + directory, object_type='entitylist')

                	rcs_client_locale = silme.io.Manager.get('file')
                	l10n_package_locale = rcs_client_locale.get_package(repo_locale + directory, object_type='entitylist')
            ...
            for bucket in stats[locale]:
                if (stats[locale][bucket]['count'] > 0):
                    stats[locale][bucket]['avg_chars'] = round(float(stats[locale][bucket]['avg_chars'])/stats[locale][bucket]['count'], 4)
                    stats[locale][bucket]['avg_perc'] =  round(float(stats[locale][bucket]['avg_perc'])/stats[locale][bucket]['count'], 4)

    # Analysis completed, write down json
    jsonfile = open('stats_gaia.json', "w")
